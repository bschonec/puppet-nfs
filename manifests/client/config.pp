  ...
  if $nfs::client::nfs_v4 {
    if $nfs::defaults_file != undef {
      if $nfs::client_gssd_options != '' {
        $_gssd1_aug = ["set ${facts['nfs::client_gssdopt_name']} \"'${facts['nfs::client_gssd_options']}'\""]
      } else {
        $_gssd1_aug = undef
      }
      ...
      augeas { $nfs::defaults_file:
        context => "/files/${facts['nfs::defaults_file']}",
    ...
    if ($nfs::client_rpcbind_config != undef) and ($nfs::client_rpcbind_optname != undef) and ($nfs::client_rpcbind_opts != undef) {
      augeas { $nfs::client_rpcbind_config:
        ...
        lens    => 'Shellvars.lns',
        context => "/files/${facts['nfs::client_rpcbind_config']}",
        changes => "set ${facts['nfs::client_rpcbind_optname']} \"'${facts['nfs::client_rpcbind_opts']}'\"",
      }
    }
    ...
    if ( $nfs::server_enabled == false ) or ( $nfs::server_enabled == true and $nfs::nfs_v4 == false ) {
      if $nfs::idmapd_file != undef {
        ...
        if $nfs::nfs_v4_idmap_cache != 0 {
          $_cache = "set General/Cache-Expiration ${facts['nfs::nfs_v4_idmap_cache']}"
        } else {
          $_cache = undef
        }

        if $nfs::manage_nfs_v4_idmap_nobody_mapping {
          $_user = "set Mapping/Nobody-User ${facts['nfs::nfs_v4_idmap_nobody_user']}"
          $_group = "set Mapping/Nobody-Group ${facts['nfs::nfs_v4_idmap_nobody_group']}"
        } else {
          $_user = undef
          $_group = undef
        }

        $changes = ["set General/Domain ${facts['nfs::nfs_v4_idmap_domain']}", $_aug_localrealm, $_cache, $_user, $_group]
        $filtered_changes = filter($changes) |$val| { $val =~ NotUndef }

        augeas { $nfs::idmapd_file:
          context => "/files/${facts['nfs::idmapd_file']}",
          lens    => 'Puppet.lns',

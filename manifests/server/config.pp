  ...
  if $nfs::nfs_v4 {
    if $nfs::nfsv4_bindmount_enable {
      concat::fragment { 'nfs_exports_root':
        target  => $nfs::exports_file,
        content => "${facts['nfs::server::nfs_v4_export_root']} ${facts['nfs::server::nfs_v4_export_root_clients']}\n",
        order   => 2,
      }
    }
    ...
    $changes = ["set General/Domain ${facts['nfs::server::nfs_v4_idmap_domain']}", $_aug_localrealm, $_cache, $_user, $_group]
